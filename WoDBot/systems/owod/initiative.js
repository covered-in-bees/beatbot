...
module.exports = {
    ...
    execute(message, args) {
        ...
        for (let character of party) {
            let initiative = character.initiative ? parseInt(character.initiative) : 0;
            characters.push(new Character(character.name, parseInt(character.wits) + parseInt(character.dexterity) + initiative));
        }
        if (args.length > 0) {
